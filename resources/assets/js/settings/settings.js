...
listen("click", ".gemini-enable", function () {
    $(".gemini-div").toggleClass("d-none");
});
...
listen("submit", "#UserCredentialsSettings", function () {
    ...
    if ($("#geminiEnable").prop("checked")) {
        if ($("#geminiApiKey").val().trim().length === 0) {
            displayErrorMessage(Lang.get("js.gemini_key"));
            return false;
        } else if ($("#geminiModel").val().trim().length === 0) {
            displayErrorMessage(Lang.get("js.gemini_model"));
            return false;
        }
    }
