<div>
    ...
    <div class="px-50">
        ...
        <div class="offcanvas offcanvas-start d-lg-none" tabindex="-1" id="filterOffcanvas" aria-labelledby="filterOffcanvasLabel" wire:ignore.self>
            <div class="offcanvas-header border-bottom border-secondary">
                <h5 class="offcanvas-title" id="filterOffcanvasLabel">{{ __('messages.common.filter') }}</h5>
                ...
                <div class="items-tabs p-0">
                    <div class="row mx-0 mb-30">
                        <div class="col-4 ps-0 px-1">
                            <input type="number" min="0" wire:model.defer="minPrice" class="form-control" placeholder="{{ __('messages.whatsapp_stores_templates.min') }}" oninput="this.value = this.value.replace(/[^0-9]/g, '')" />
                        </div>
                        <div class="col-4 px-1">
                            <input type="number" min="1" wire:model.defer="maxPrice" class="form-control" placeholder="{{ __('messages.whatsapp_stores_templates.max') }}" oninput="this.value = this.value.replace(/[^0-9]/g, '')" />
                        </div>
                        <div class="col-4 pe-0 px-1">
                            <button wire:click="applyPriceFilter" class="apply-btn btn btn-primary w-100" data-bs-dismiss="offcanvas">{{ __('messages.whatsapp_stores_templates.apply') }}</button>
                        </div>
                    </div>
                    <div class="mb-30">
                        <div class="heading-text mb-4"><h3>{{ __('messages.whatsapp_stores_templates.date_posted') }}</h3></div>
                        <div>
                            <div class="form-check mb-2" wire:key="m-date-3d"><input class="form-check-input" type="radio" name="m-dateFilter" id="m-date1" wire:model.live="dateFilter" value="3_days" /><label class="form-check-label fs-16 fw-5" for="m-date1">3 {{ __('messages.whatsapp_stores_templates.days_ago') }}</label></div>
                            <div class="form-check mb-2" wire:key="m-date-1w"><input class="form-check-input" type="radio" name="m-dateFilter" id="m-date2" wire:model.live="dateFilter" value="1_week" /><label class="form-check-label fs-16 fw-5" for="m-date2">1 {{ __('messages.whatsapp_stores_templates.week_ago') }}</label></div>
                            <div class="form-check mb-2" wire:key="m-date-1m"><input class="form-check-input" type="radio" name="m-dateFilter" id="m-date3" wire:model.live="dateFilter" value="1_month" /><label class="form-check-label fs-16 fw-5" for="m-date3">1 {{ __('messages.whatsapp_stores_templates.month_ago') }}</label></div>
                            <div class="form-check mb-2" wire:key="m-date-6m"><input class="form-check-input" type="radio" name="m-dateFilter" id="m-date4" wire:model.live="dateFilter" value="6_months" /><label class="form-check-label fs-16 fw-5" for="m-date4">6 {{ __('messages.whatsapp_stores_templates.months_ago') }}</label></div>
                            <div class="form-check mb-2" wire:key="m-date-1y"><input class="form-check-input" type="radio" name="m-dateFilter" id="m-date5" wire:model.live="dateFilter" value="1_year" /><label class="form-check-label fs-16 fw-5" for="m-date5">1 {{ __('messages.whatsapp_stores_templates.year_ago') }}</label></div>
                        </div>
                    </div>
                    <div class="mb-30">
                        <div class="heading-text mb-4"><h3>{{ __('messages.whatsapp_stores_templates.search_price_range') }}</h3></div>
                    ...
                    <div class="date-posted">
                        <div class="heading-text mb-20"><h3 class="mb-0 fs-18 fw-medium">{{ __('messages.whatsapp_stores_templates.all_categories') }}</h3></div>
                        <div>
                            @foreach ($categories as $category)
                                <div class="form-check mb-2" wire:key="m-cat-{{ $category->id }}">
                                    <input class="form-check-input" type="checkbox" wire:model.live="categoryFilter" value="{{ $category->id }}" id="m-cat-{{ $category->id }}" />
                                    <label class="form-check-label fs-16 fw-medium" for="m-cat-{{ $category->id }}">{{ $category->name }}</label>
                                </div>
                            @endforeach
                        </div>
                    </div>
                    <div class="mt-4">
                        <button type="button" wire:click="resetFilters" class="apply-btn btn btn-primary w-100 mb-3" data-bs-dismiss="offcanvas">{{ __('messages.whatsapp_stores_templates.reset_filters') }}</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="px-50 mb-3 d-none d-lg-block">
                    ...
                    <div class="custom-select-container position-relative">
                        <div class="custom-select">
                            <div
                                ...
                                <div class="custom-arrow-select position-absolute d-flex align-items-center {{ app()->getLocale() == 'ar' || app()->getLocale() == 'fa' ? 'rtl-arrow' : '' }}">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"
                                        viewBox="0 0 20 20" fill="none">
                                        <path fill-rule="evenodd" clip-rule="evenodd"
                                            d="M4.41098 6.91098C4.25476 7.06725 4.16699 7.27918 4.16699 7.50015C4.16699 7.72112 4.25476 7.93304 4.41098 8.08931L9.41098 13.0893C9.56725 13.2455 9.77918 13.3333 10.0001 13.3333C10.2211 13.3333 10.433 13.2455 10.5893 13.0893L15.5893 8.08931C15.7411 7.93215 15.8251 7.72164 15.8232 7.50315C15.8213 7.28465 15.7337 7.07564 15.5792 6.92113C15.4247 6.76663 15.2156 6.67898 14.9971 6.67709C14.7787 6.67519 14.5681 6.75918 14.411 6.91098L10.0001 11.3218L5.58931 6.91098C5.43304 6.75476 5.22112 6.66699 5.00015 6.66699C4.77918 6.66699 4.56725 6.75476 4.41098 6.91098Z"
                ...
                <div class="col-lg-3 col-sm-6">
                    <div class="h-100 d-flex justify-content-between flex-column product-card bg-white">
                        ...
                        <a class="fs-20 fw-7 lh-sm mb-0 " href="{{ route('whatsapp.store.product.details', [$whatsappStore->url_alias, $product->id]) }}">
                            <span class="currency_icon selling_price text-black">{{ currencyFormat($product->selling_price, 2, $product->currency->currency_code) }}</span>
                            @if ($product->net_price)
                                <del class="fs-14 fw-6 text-gray-200 text-nowrap">{{ currencyFormat($product->net_price, 2, $product->currency->currency_code) }}</del>
                            @endif
                        </a>
                        <button
