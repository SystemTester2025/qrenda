            ...
            @if ((isset($managesection) && $managesection['galleries']) || empty($managesection))
                @if(checkFeature('gallery') && $vcard->gallery->count())
                    <section class="client-section">
                                        ...
                                        <div class="client-box w-100 h-100 ">
                                            <div class="client-img">
                                                @if($file->type == App\Models\Gallery::TYPE_IMAGE)
                                                    <a href="{{$file->gallery_image}}" data-lightbox="gallery-images-all"><img src="{{ $file->gallery_image }}"
                                                ...
                                                @else
                                                    <iframe src="https://www.youtube.com/embed/{{ YoutubeID($file->link) }}"
                                                        class="w-100 h-100 object-fit-cover" allowfullscreen>
                                                    </iframe>
                                                @endif
                                            </div>
                                        </div>
                        ...
                        @if ($groupedGallery->isNotEmpty())
                            @foreach ($groupedGallery as $categoryName => $galleryItems)
                                <div class="gallery-filter-panel" data-gallery-panel="{{ \Illuminate\Support\Str::slug($categoryName, '-') }}">
                                    <div class="row">
                                        @foreach($galleryItems as $file)
                                            ...
                                            <div class="col-md-3 col-6  mt-3">
                                                <div class="client-box w-100 h-100 ">
                                                    <div class="client-img">
                                                        @if($file->type == App\Models\Gallery::TYPE_IMAGE)
                                                            <a href="{{$file->gallery_image}}" data-lightbox="gallery-images-{{ \Illuminate\Support\Str::slug($categoryName, '-') }}"><img src="{{ $file->gallery_image }}"
                                                                                                                                class="w-100 h-100 object-fit-cover rounded"></a>
                                                        @elseif($file->type == App\Models\Gallery::TYPE_FILE)
                                                            <a id="file_url" href="{{$file->gallery_image}}"
                                                            class="gallery-link gallery-file-link" target="_blank">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        $(document).on('click', '.gallery-filter-btn', function () {
