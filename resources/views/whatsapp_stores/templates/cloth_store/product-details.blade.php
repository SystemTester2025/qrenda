        ...
        <nav class="navbar navbar-expand-lg px-50 position-relative">
            <div class="container-fluid p-0 gap-2 gap-lg-4 flex-nowrap">
                <div class="d-flex align-items-center gap-3 overflow-hidden">
                    ...
                    <span class="fw-6 fs-18 text-truncate"><a href="{{ route('whatsapp.store.show', $whatsappStore->url_alias) }}"
                        ...
                        <div class="details d-flex flex-column justify-content-between h-100">
                            ...
                            <input type="hidden" value="{{ $product->images_url[0] ?? '' }}"
                                ...
                                <p class="fs-28 fw-5 mb-30">
                                    ...
                                    <del class="fs-18 fw-6 text-gray-200">{{ currencyFormat($product->net_price, 2, $product->currency->currency_code) }}</del>
                ...
                @foreach ($whatsappStore->products()->orderByRaw('sort IS NULL, sort ASC')->orderByDesc('created_at')->get() as $product)
                    <div>
                        <a href="{{ route('whatsapp.store.product.details', [$whatsappStore->url_alias, $product->id]) }}"
                            class="d-block h-100 product-card" style="color: #212529;">
                            <div class=" h-100 d-flex flex-column">
                                ...
                                <div class="product-desc h-100">
                                    <input type="hidden" class="product-category" value="{{ $product->category->name }}" />
                                    <input type="hidden" value="{{ $product->available_stock }}"
                                        class="available-stock">
                                    <p class="fs-20 fw-5 mb-2 product-name">{{ $product->name }}</p>
                                    <div class="d-flex justify-content-between align-items-center">
                                        ...
                                        @if ($product->available_stock > 0)
                                            <button
                                                class="@if($product->available_stock == 0) disabled @endif btn btn-primary d-flex gap-2 align-items-center fs-12 fw-6 ms-auto addToCartBtn add-to-cart-min-w-127px"
                                                data-id="{{ $product->id }}">
