...
listenClick(".addToCartBtn", function (event) {
  ...
  var numericPrice = Number(price.replace(/,/g, ""));
  var product = {
    id: $(this).data("id"),
...
listenClick(".delete-btn", function () {
  ...
  if (cart["store_".concat(storeId)] && cart["store_".concat(storeId)][productId]) {
    ...
    productCount(storeId);
    var newGrandTotal = (_cart$grand_total2 = (_cart = cart["store_".concat(storeId)]) === null || _cart === void 0 ? void 0 : _cart.grand_total) !== null && _cart$grand_total2 !== void 0 ? _cart$grand_total2 : 0;
    var discountPercent = Number($("#storeDiscount").val()) || 0;
    var discountAmount = newGrandTotal * discountPercent / 100;
    var finalTotal = newGrandTotal - discountAmount;
...
listenClick(".plus-btn", function () {
  ...
  if (storeCart && storeCart[productId]) {
    ...
    var discountPercent = Number($("#storeDiscount").val()) || 0;
    var discountAmount = storeCart.grand_total * discountPercent / 100;
    var finalTotal = storeCart.grand_total - discountAmount;
...
listenSubmit("#orderForm", function (event) {
  ...
  var orderDetails = $(this).serialize() + "&wp_store_id=" + storeId + "&discount=" + discountPercent + "&discount_amount=" + discountAmount.toFixed(2) + "&grand_total=" + finalTotal.toFixed(2) + "&products=" + encodeURIComponent(JSON.stringify(products)) + "&language=" + lang;
...
function prepareAndSendWpMessage(order) {
  ...
  message += "\n".concat(Lang.get("js.total"), ": ").concat(subTotal.toFixed(2));
  if (order.discount_amount && parseFloat(order.discount_amount) > 0) {
    message += "\n".concat(Lang.get("js.discount"), ": ").concat(order.discount_amount);
  }
