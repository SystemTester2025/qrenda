...
listenChange(".file-validation", function () {
  ...
  var APP_IMAGE_MAX_BYTES = 5 * 1024 * 1024;
  if (window.APP_UPLOAD_MAX_BYTES < APP_IMAGE_MAX_BYTES && file.size > window.APP_UPLOAD_MAX_BYTES) {
    var maxMB = Math.floor(window.APP_UPLOAD_MAX_BYTES / 1024 / 1024);
    displayErrorMessage(Lang.get('js.exceeds_allowed_upload_size', {
      size: maxMB
    }));
    $(this).val("");
    return;
...
$(document).on('click', '.sidebar-btn', function () {
  setTimeout(function () {
    var isCollapsed = $('#sidebar').hasClass('collapsed-menu');
...
document.addEventListener("DOMContentLoaded", function () {
  if (document.cookie.includes("banner-section=1")) {
    $(".banner-cookie").addClass("d-none").removeClass("d-block");
  } else {
    $(".banner-cookie").removeClass("d-none").addClass("d-block");
  }
});
listenClick(".close-btn", function () {
