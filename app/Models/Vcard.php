...
class Vcard extends Model implements HasMedia
{
    ...
    protected $casts = [
        ...
        'enable_business_directory' => 'integer',
    ];
     ...
     * @var string[]
     */
    protected $appends = ['profile_url', 'cover_url', 'profile_url_base64', 'full_name', 'favicon_url', 'business_directory_url'];
    ...
    const QRCODE_PATH = 'vcards/qr_codes';
    ...
    public function getFaviconUrlAttribute(): string
    {
        ...
        static $defaultFaviconUrl;

        if ($defaultFaviconUrl === null) {
    ...
    public function getBusinessDirectoryUrlAttribute(): string
    {
        if ((int)$this->cover_type === self::IMAGE) {
            $cover = $this->getMedia(self::COVER_PATH)->first();
        ...
        $profile = $this->getMedia(self::PROFILE_PATH)->first();

        if ($profile) {
            return $profile->getFullUrl();
        }

        return asset('images/vcard-default.png');
    }
    ...
    {
        return $this->belongsToMany(User::class, 'vcard_user')->withTimestamps();
    }
    ...
    {
        return $this->hasMany(GalleryCategory::class, 'vcard_id');
    }
    ...
    public function isEditableBy(?User $user = null): bool
    {
        ...
        if ($this->tenant_id !== $user->tenant_id || ! $user->canEditVcards()) {
            return false;
        }
