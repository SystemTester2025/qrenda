...
if (!function_exists('isOrganisationOwner')) {
    function isOrganisationOwner()
    {
        return Auth::check() && Auth::user()->is_organisation == 1 && empty(Auth::user()->organisation_id);
    }
}

if (!function_exists('isOrganisationUser')) {
    function isOrganisationUser()
    {
        return Auth::check() && !empty(Auth::user()->organisation_id);
    }
}

if (!function_exists('getOrganisationUserPlanLimit')) {
    function getOrganisationUserPlanLimit(?string $tenantId = null): int
    {
        $tenantId = $tenantId ?: (Auth::check() ? getLogInTenantId() : null);

        if (empty($tenantId)) {
            return 0;
        }

        static $organisationUserPlanLimits = [];

        if (array_key_exists($tenantId, $organisationUserPlanLimits)) {
            return $organisationUserPlanLimits[$tenantId];
        }

        $subscription = Subscription::with(['plan.planFeature'])
            ->whereTenantId($tenantId)
            ->where('status', Subscription::ACTIVE)
            ->latest()
            ->first();

        if (! $subscription || ! $subscription->plan || ! $subscription->plan->planFeature) {
            return $organisationUserPlanLimits[$tenantId] = 0;
        }

        if ((int) $subscription->plan->planFeature->organization_users !== 1) {
            return $organisationUserPlanLimits[$tenantId] = 0;
        }

        return $organisationUserPlanLimits[$tenantId] = max(0, (int) ($subscription->plan->no_of_organization_users ?? 0));
    }
}

if (!function_exists('canManageOrganisationUsers')) {
    function canManageOrganisationUsers($user = null): bool
    {
        $user = $user ?: Auth::user();
        ...
        return ! empty($user)
            ...
            && getOrganisationUserPlanLimit($user->tenant_id) > 0;
    }
}

if (!function_exists('getOrganisationUserCount')) {
    function getOrganisationUserCount(?int $organisationId = null, ?string $tenantId = null): int
    {
        if (empty($organisationId) && Auth::check()) {
        ...
        if (empty($organisationId) || empty($tenantId)) {
            return 0;
        }
...
if (!function_exists('getActiveOrganisationUserCount')) {
    function getActiveOrganisationUserCount(?int $organisationId = null, ?string $tenantId = null): int
    {
        if (empty($organisationId) && Auth::check()) {
            $organisationId = getLogInUser()->organisation_id ?: getLogInUserId();
        }
...
if (!function_exists('canAddMoreOrganisationUsers')) {
    function canAddMoreOrganisationUsers($user = null): bool
    {
        $user = $user ?: Auth::user();

        if (! canManageOrganisationUsers($user)) {
            return false;
        }

        return getActiveOrganisationUserCount($user->id, $user->tenant_id) < getOrganisationUserPlanLimit($user->tenant_id);
    }
}

if (!function_exists('enforceOrganisationUserPlanLimitByTenant')) {
    function enforceOrganisationUserPlanLimitByTenant(?string $tenantId): void
    {
        if (empty($tenantId)) {
            return;
        }

        $organisationOwner = User::where('tenant_id', $tenantId)
        ...
        $limit = getOrganisationUserPlanLimit($tenantId);

        $baseQuery = User::where('tenant_id', $tenantId)
            ->where('organisation_id', $organisationOwner->id)
            ->where('is_organisation', false);

        if ($limit <= 0) {
            $baseQuery->update(['is_active' => User::DEACTIVATE]);
        ...
        $keepActiveUserIds = (clone $activeUsersQuery)
        ...
        (clone $activeUsersQuery)
            ->when(! empty($keepActiveUserIds), function ($query) use ($keepActiveUserIds) {
                $query->whereNotIn('id', $keepActiveUserIds);
            })
            ->when(empty($keepActiveUserIds), function ($query) {
                return $query;
            })
            ->update(['is_active' => User::DEACTIVATE]);

        if (empty($keepActiveUserIds)) {
            return;
        }
    }
}
...
if (!function_exists('getPlanFeature')) {
    function getPlanFeature($plan): array
    {
        ...
        if (! isOrganisationOwner()) {
            $except[] = 'organization_users';
        }

        return Arr::except($planFeatures, $except);
    }
}
...
if (!function_exists('getCurrentSubscription')) {
    function getCurrentSubscription()
    {
        ...
        $tenantId = getLogInTenantId();
        if (array_key_exists($tenantId, $currentSubscriptions)) {
            return $currentSubscriptions[$tenantId];
        }
        ...
        return $currentSubscriptions[$tenantId] = $subscription;
    }
}

if (!function_exists('getUserCurrentSubscription')) {
    function getUserCurrentSubscription(?User $user = null)
    {
        $user = $user ?: Auth::user();
        ...
        return Subscription::with(['plan.planFeature'])
...
if (!function_exists('isOrganisationStaffSubscriptionExpired')) {
    function isOrganisationStaffSubscriptionExpired(?User $user = null): bool
    {
        $user = $user ?: Auth::user();
        ...
        $subscription = getUserCurrentSubscription($user);

        return empty($subscription) || $subscription->isExpired();
    }
}
...
if (!function_exists('checkTotalVcard')) {
    function checkTotalVcard(): bool
    {
        ...
        if (!empty($subscription)) {
            ...
            if ($makeVcard && isOrganisationUser()) {
                $userVcards = Vcard::whereHas('assignedUser', function ($q) use ($user) {
                    $q->where('users.id', $user->id);
                })->count();
                $userVcardLimit = $user->organisationUserPermission ? $user->organisationUserPermission->no_of_vcards : 0;

                if ($userVcardLimit === null) {
                    $userVcardLimit = $subscription->no_of_vcards;
                }

                $makeVcard = $userVcardLimit > $userVcards;
            }
        }
...
if (!function_exists('checkTotalWhatsappStore')){
    function checkTotalWhatsappStore(): bool
    {
        ...
        if (!empty($subscription)) {
            ...
            $whatsappStore = $subscription->no_of_whatsapp_store > $totalWhatsappStore;

            if ($whatsappStore && isOrganisationUser()) {
                $userWhatsappStores = WhatsappStore::whereHas('assignedUser', function ($q) use ($user) {
                    $q->where('users.id', $user->id);
                })->count();
                $userWhatsappStoreLimit = $user->organisationUserPermission ? $user->organisationUserPermission->no_of_whatsapp_store : 0;

                if ($userWhatsappStoreLimit === null) {
                    $userWhatsappStoreLimit = $subscription->no_of_whatsapp_store;
                }

                $whatsappStore = $userWhatsappStoreLimit > $userWhatsappStores;
            }
        }
...
if (!function_exists('hasActiveSubscription')) {
    function hasActiveSubscription()
    {
        ...
        if (array_key_exists($tenantId, $hasActiveSubscription)) {
            return $hasActiveSubscription[$tenantId];
        }
        ...
        return $hasActiveSubscription[$tenantId] = $subscription && !$subscription->isExpired();
    }
}
...
if (!function_exists('isVisitorLimitExceeded')) {
    function isVisitorLimitExceeded($vcardId): bool
    {
        ...
        if (!$plan->planFeature || $plan->planFeature->visitors != 1) {
            return false;
        }

        $limit = $plan->no_of_visitors;
        ...
        $visitorCount = Analytic::where('vcard_id', $vcardId)
            ->whereBetween('created_at', [$subscription->created_at, $subscription->ends_at])
            ->distinct('session')
            ->count('session');

        return $visitorCount > $limit;
    }
}
